...
const ClientAppointments = () => {
  ...
  const [payConfirm, setPayConfirm] = useState({ open: false, appt: null });
  ...
  const handlePay = async (appointment_id) => {
    ...
    const timeout = setTimeout(() => {
      if (!finished) {
        ...
        toast.error("Tiempo de espera excedido. Intenta nuevamente.");
      }
    }, 10000);
    try {
      const token = localStorage.getItem("token");
      const res = await fetch(
        `http://localhost:4000/pay-appointment/${appointment_id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json", token },
        }
      ...
      if (res.ok) {
        // Actualizar la lista de citas después de pagar
        const updated = await fetch("http://localhost:4000/my-appointments", {
          method: "GET",
          headers: { token },
        });
        ...
        toast.error("No se pudo pagar la cita");
      }
    } catch (error) {
      console.error(error);
      toast.error("Error al pagar la cita");
    } finally {
      finished = true;
  ...
  // para saber si una cita ya tiene reseña
          ...
          {appointments.map((appt) => (
            ...
            >
              ...
              <div className="mt-4 border-t pt-4">
                ...
                <div className="mt-4 flex gap-3 justify-between items-end">
                  {appt.status === "RECHAZADA" && (
                    <p className="bg-red-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-red-600">
                      Su cita fue Rechazada, por favor escoja otra fecha
                    </p>
                  )}
                    ...
                    !hasReview(appt.appointment_id) && (
                      <>
                        <div className="flex gap-3">
                          ...
                          <button
                            className="bg-blue-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-blue-600 cursor-pointer"
                            onClick={() => setPayConfirm({ open: true, appt })}
                          >
                            Pagar
                          </button>
                          <button
                            className="bg-green-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-green-600 cursor-pointer"
                  ...
                  {appt.status === "PAGADA" &&
                    !hasReview(appt.appointment_id) && (
                      <>
                        ...
                        <Link
                          to={`/reclamo?appointment_id=${appt.appointment_id}`}
                          className="inline-block bg-red-400 text-white px-4 py-2 rounded-md font-semibold hover:bg-red-500 transition-colors cursor-pointer"
                        >
                          Reclamar
                        </Link>
                      </>
                    )}
                  {appt.status === "RECLAMO EN REVISIÓN" && (
                    <span className="inline-block bg-yellow-300 text-[#1E3A8A] px-4 py-2 rounded-md font-semibold">
                      En revisión de reclamo
                    </span>
                  )}
                  {appt.status === "PAGADA" &&
                    hasReview(appt.appointment_id) && (
                      <span className="bg-gray-300 text-white px-4 py-2 rounded-md font-semibold cursor-not-allowed">
                        Reseña enviada
                      </span>
                    )}
                  {appt.status === "PENDIENTE" && (
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-md w-full">
            <h3 className="text-lg font-bold text-[#1E3A8A]">
              {confirm.type === "reject" ? "Cancelar Cita" : "Cancelar Cita"}
            </h3>
            <p className="mt-2 text-gray-700">
              ¿Estás seguro que deseas{" "}
              {confirm.type === "reject" ? "CANCELAR" : "CANCELAR"} esta cita?
            </p>
